...
static void (*_t0_func)(void) = 0;
static void (*_t1_func)(void) = 0;
static void (*_t2_func)(void) = 0;

void timer0(uint8_t prescaler, uint8_t ticks, void (*f)(void)) {
...
ISR(TIMER0_COMPA_vect) {
	if (_t0_func) _t0_func();
}
#endif

void timer1(uint8_t prescaler, uint16_t ticks, void (*f)(void)) {
	TIMSK1 &= ~(_BV(OCIE1A));
	_t1_func = f;
	OCR1A = ticks;
	TCCR1A = 0;
...
ISR(TIMER1_COMPA_vect) {
	if (_t1_func) _t1_func();
}
#endif

void timer2(uint8_t prescaler, uint8_t ticks, void (*f)(void)) {
	TIMSK2 &= ~(_BV(OCIE2A));
	_t2_func = f;
	OCR2A = ticks;
	ASSR = 0;
